...
function Generate() {
  ...
  const allowGenerate = () => {
    ...
    if (inputMode === "Detailed") {
      return detailedData.description.trim().length > 0;
    }
    if (inputMode === "Simplified") {
      return simplifiedData.systemType.trim().length > 0
        && simplifiedData.dataPurpose.trim().length > 0;
    }
    if (inputMode === "Upload") {
      ...
      <div className="w-full max-w-4xl">
        ...
        <div className="flex flex-col space-y-3 mt-6">
          <button
            ...
            className={`w-full p-3 rounded-lg font-semibold border-b-4 transition-all ${!allowGenerate()
